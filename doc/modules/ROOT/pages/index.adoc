...
[NOTE]
====
You need to have a running Hazelcast cluster before using client. `com.hazelcast.guide.HazelcastServer` will 
start an instance with default configuration. When you start it multiple times, you will see Hazelcast members starting,
finding each other and forming a cluster. You can also try Hazelcast CLI[https://github.com/hazelcast/hazelcast-command-line]
